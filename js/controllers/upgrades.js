...
export const UpgradesController = {
    renderUpgrades(app) {
        ...
        list.innerHTML += app.upgCardHTML("🔍 Market Tier Limit", tierDesc, isMaxTier, nextTierCost, "GameApp.buyMarketTier(event)");
        ...
        Object.keys(GameState.products).forEach(pKey => {
            ...
            if (prod.unlockCost > 0) {
                list.innerHTML += app.upgCardHTML(`${prod.emoji} Unlock ${prod.name}s`, prod.unlocked ? "Unlocked & active" : `Adds ${prod.name} to rotation`, prod.unlocked, prod.unlockCost, `GameApp.buyProduct('${pKey}', ${prod.unlockCost}, event)`);
            }
        });
    },
